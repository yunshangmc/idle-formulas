...
export default function KeyBoardHandler({ state, updateState, popup }) {
    ...
    if (e.repeat) {
      updateState({
        ...
        newValue: {
          ...
          delay: state.holdAction
            ? Math.min(
                state.holdAction.delay !== undefined
  ...
  return (
    <>
      <Hotkeys
        keyName="1,2,3,4,5,num_1,num_2,num_3,num_4,num_5"
        disabled={state.settings.hotkeyApplyFormula === "OFF"}
        onKeyDown={hotkeyApplyFormulaDown}
        onKeyUp={hotkeyApplyFormulaUp}
        allowRepeat={true}
      />
      <Hotkeys
        keyName="b"
        disabled={state.settings.hotkeyBasicReset === "OFF"}
        onKeyDown={hotkeyBasicReset}
      />
      <Hotkeys
        keyName="x"
        disabled={state.settings.hotkeyXReset === "OFF"}
        onKeyDown={hotkeyXReset}
      />
      <Hotkeys
        keyName="a"
        disabled={state.settings.hotkeyAlphaReset === "OFF"}
        onKeyDown={hotkeyAlphaReset}
      />
      <Hotkeys
        keyName="t"
        disabled={state.settings.hotkeyToggleAuto === "OFF"}
        onKeyDown={hotkeyToggleAuto}
      />
      <Hotkeys
        keyName="c"
        disabled={state.settings.hotkeyAbortRun === "OFF"}
        onKeyDown={hotkeyAbortRun}
      />
    </>
  );
}
