...
export const isLockedByChallenge = (state, formula) =>
  (state.activeChallenges.SIMPLEONLY && formula.complex) ||
  (state.activeChallenges.COMPLEX && !formula.complex && !formula.isBasic) ||
  (state.activeChallenges.NEWONLY &&
...
export default function FormulaButton({
  ...
  const applyFormula = (formula, evt) => {
    if (
      ...
      state.settings.valueReduction === "ON" &&
      0.9999 * state.xValue[formula.targetLevel] >
        formula.applyFormula(
          state.formulaEfficiency[formula.targetLevel],
          state.xValue,
          state
        )
    ) {
      popup.confirm(
        <>
          Applying this formula will reduce the value. Are you sure?
          <br />
          If you confirm, this pop-up gets disabled until your next Reset.
        </>,
        () => {
        ...
        {state.alphaUpgrades.AAPP && (
          <td>
            <input
              ...
              type="checkbox"
              checked={state.autoApply[myIndex]}
              readOnly
            ></input>
          </td>
        )}
  ...
  if (
    state.activeChallenges.SMALLINV &&
    !state.anyFormulaUsed &&
    state.formulaBought[formula.formulaName]
  ) {
    applyCost = 0;
    applyNeed = 0;
  ...
  var tooltip =
    applyCost >= applyNeed
      ? (state.alphaUpgrades.FREF ? "Need" : "Cost") +
        ": x=" +
        formatNumber(applyCost, state.settings.numberFormat)
      : "Need: x=" + formatNumber(applyNeed, state.settings.numberFormat);
  const delimiter = state.settings.shopPrices ? " / " : "\n";
  var tooltipplus =
    tooltip +
    (formula && formula.explanation ? delimiter + formula.explanation : "");
  ...
  if (applyNeed <= state.xValue[0] && applyCost <= state.xValue[0]) {
    ...
    if (
      context === "my" &&
      state.xValue[formula.targetLevel] >= 0 &&
      formula.applyFormula(
        state.formulaEfficiency[formula.targetLevel],
        state.xValue,
        state
      ) >
        1.05 * state.xValue[formula.targetLevel]
    ) {
  ...
  if (formula.numberFormat !== state.settings.numberFormat)
    formula.description =
      formula.descriptions?.[state.settings.numberFormat] ||
      formula.description;
  ...
  const mouseHandler = (e) => {
    switch (e.type) {
      ...
      case "touchend":
        if (
          state.holdAction &&
          state.holdAction.formulaName === formula.formulaName
        ) {
  ...
  if (context === "my") {
        ...
        <td align="left" className="block" style={{ width: "auto" }}>
          <button
            ...
            disabled={
              ...
              !state.formulaUnlocked[formulaName] ||
              (applyNeed && state.xValue[0] < applyNeed) ||
              (applyCost && state.xValue[0] < applyCost)
            }
            ...
            onTouchEnd={mouseHandler}
          >
            {formula.description}
          </button>
        </td>
        <td>
          {!state.formulaUnlocked[formulaName] && lockedByChallenge && (
            <>&nbsp;Locked</>
          )}
          {!state.formulaUnlocked[formulaName] && !lockedByChallenge && (
            <>
              &nbsp;Unlock: x=
              {formatNumber(
                formula.unlockCost * formula.unlockMultiplier,
          ...
          {state.formulaUnlocked[formulaName] &&
            ...
            state.xValue[0] < applyCost && (
              <>
                &nbsp;{state.alphaUpgrades.FREF ? "Need" : "Cost"}: x=
                {formatNumber(applyCost, state.settings.numberFormat)}
              </>
            )}
          {state.formulaUnlocked[formulaName] &&
            !!applyNeed &&
            state.xValue[0] < applyNeed && (
              <>
                &nbsp;Need: x=
                {formatNumber(applyNeed, state.settings.numberFormat)}
              </>
            )}
          {state.formulaUnlocked[formulaName] &&
            state.xValue[0] >= applyNeed &&
            state.xValue[0] >= applyCost &&
            !state.formulaUsed[formulaName] && <>&nbsp;Click to apply!</>}
          {!state.formulaUsed[formulaName] && (
  ...
  } else if (lockedByChallenge) {
    //LOCKED BY CHALLENGE (=>HIDDEN)
    return undefined;
  } else if (
    formula.effectLevel > state.highestXTier ||
    ...
    state.settings.shopFilter === "EDIT" &&
    state.mailsCompleted["Favorites"] !== undefined
  ) {
    //Edit Formula Display Type
    return (
      <tr>
        <td align="left" className="block" style={{ width: "auto" }}>
          <button
            ...
            style={{
              ...
              fontWeight: buttonBoldness,
              fontStyle: buttonItalicity,
            }}
            onClick={() => changeFavorite(formula)}
            onMouseDown={(e) => {
              e.preventDefault();
            }}
          >
        ...
        <td>{spaces()}</td>
        <td>
          {state.shopFavorites[state.highestXTier][formula.formulaName] ===
            1 && <b>Favorite</b>}
          {state.shopFavorites[state.highestXTier][formula.formulaName] ===
            -1 && <i>Hidden</i>}
        </td>
  ...
  } else if (state.formulaBought[formulaName]) {
    //EQUIPPED
    return (
      <tr>
        <td align="left" className="block" style={{ width: "auto" }}>
        ...
        <td>{spaces()}</td>
        <td>{state.settings.shopPrices === "ON" && tooltip}</td>
      </tr>
    );
  } else if (state.formulaUnlocked[formulaName]) {
    //GET BUTTON
    return (
      <tr>
        <td align="left" className="block" style={{ width: "auto" }}>
    ...
    return (
      <tr>
        ...
        <td>
          {!formula.isFree ? (
            <>
              Unlock{state.alphaUpgrades.AUNL ? "s" : ""}{" "}
              {state.alphaUpgrades.AUNL ? "at" : "for"} x=
              {formatNumber(
                formula.unlockCost * formula.unlockMultiplier,
                state.settings.numberFormat
              )}
