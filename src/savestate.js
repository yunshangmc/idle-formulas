...
import {
  getUnlockMultiplier,
  isLockedByChallenge,
} from "./formulas/FormulaButton";
import { calcStoneResultForX } from "./alpha/AlphaStonesTab";
import {
  startingStones,
  stoneTable,
  stoneList,
} from "./alpha/AlphaStoneDictionary";
...
export const getSaveGame = () => {
  ...
  if (!savedgame) {
    return {
      ...structuredClone(newSave),
      saveTimeStamp: Date.now(),
      calcTimeStamp: Date.now(),
      fileStartTimeStamp: Date.now(),
      destinyStartTimeStamp: Date.now(),
    };
  } else {
    ...
    if (savedgamejson.settings.autoLoad === "OFF") {
      ...
      return {
        ...
        ...savedgamejson,
        settings: {
          ...structuredClone(newSave.settings),
          ...savedgamejson.settings,
        },
        saveTimeStamp: Date.now(),
        currentEnding: newSave.currentEnding,
        justLaunched: true,
      };
    }
  }
...
export const getStartingX = (state) => {
  ...
  const fromResearch = Math.floor(
    100 * Math.pow(1.01, state.researchLevel["x"] || 0) - 100
  );
  return fromStartingStones * fromResearch;
};

export const getInventorySize = (state) => {
  if (state.activeChallenges.SMALLINV) return 1;
  else return state.alphaUpgrades.SLOT ? 4 : 3;
};
...
const giveAlphaRewards = (state) => {
  if (state.xValue[0] < alphaTarget || state.inNegativeSpace) {
    return state;
  } //No rewards
  ...
  const alphaReward =
    getAlphaRewardTier(state.xValue[0]).alpha *
    Math.pow(2, state.baseAlphaLevel);
  if (state.currentChallenge) {
    ...
    if (state.isFullyIdle && state.clearedChallenges.FULLYIDLE) {
      const newRewardInterval = getRewardInterval(
        alphaReward,
        state.currentAlphaTime,
        getGlobalMultiplier(state)
      );
      const oldMOIRewardInterval = getRewardInterval(
        state.bestIdleTimeAlpha,
        state.bestIdleTime,
        getGlobalMultiplier(state)
      );
      if (
        newRewardInterval < state.passiveAlphaInterval ||
        newRewardInterval < oldMOIRewardInterval
      ) {
        state.passiveAlphaInterval = Math.min(
          state.passiveAlphaInterval,
          newRewardInterval
        );
        state.passiveAlphaTime = Math.min(
          state.passiveAlphaTime,
          newRewardInterval
        );
    ...
    state.bestAlphaTime = Math.max(
      1000,
      Math.min(state.currentAlphaTime, state.bestAlphaTime)
    );
    state.passiveAlphaTime = Math.min(
      state.passiveAlphaTime,
      state.bestAlphaTime
    ...
    state.passiveAlphaInterval = Math.min(
      state.passiveAlphaInterval,
      getRewardInterval(1, state.bestAlphaTime, getGlobalMultiplier(state))
    );
    state.xHighScores[state.highestXTier] = Math.max(
      state.xHighScores[state.highestXTier],
      state.xValue[0]
    );
...
const rememberLoadout = (state, isManual) => {
  if (
    (state.alphaUpgrades.AREM &&
      state.settings.autoRemembererActive === "ON") ||
    isManual
  ) {
    state.myFormulas = state.equipLayouts[state.selectedLayout][
      state.highestXTier
    ].slice(0, getInventorySize(state));
    state.formulaBought = state.myFormulas.reduce(
      (a, v) => ({ ...a, [v]: true }),
      {}
    );
  } else {
...
const upgradeXTier = (state) => {
  ...
  if (state.currentChallenge) {
    ...
    state.challengeProgress[state.currentChallenge] = Math.max(
      state.challengeProgress[state.currentChallenge] || 0,
      state.highestXTier
    );
    state = updateFormulaEfficiency(state);
  }
...
export const getChallengeBonus = (state) => {
  ...
  for (let c in state.clearedChallenges) {
    if (state.clearedChallenges[c]) clearedFull++;
  }
...
export const getMaxxedResearchBonus = (state) => {
  ...
  for (let c in state.researchLevel) {
    if (state.researchLevel[c] >= 2500) maxxed++;
  }
...
const updateFormulaEfficiency = (state) => {
  ...
  state.formulaEfficiency = [
    challengeBonus * researchBonus,
    challengeBonus * researchBonus,
    challengeBonus * researchBonus,
    challengeBonus * researchBonus,
  ];
  ...
  switch (action.name) {
    case "idle":
      ...
      if (deltaMilliSeconds < 120000) {
        //Quick Computation
        if (
          state.insideChallenge ||
          state.anyFormulaUsed ||
          state.xResetCount ||
          state.highestXTier > 0
        )
        ...
        (state.settings.offlineProgress === "ON" ||
          (state.settings.offlineProgress === "ACTIVE" && !state.justLaunched))
      ) {
        //Offline Progress
        ...
        state.settings.offlineProgressPopup === "ON" ||
        (state.settings.offlineProgressPopup === "LAUNCH" && state.justLaunched)
      ) {
      ...
      if (state.holdAction?.type === "ApplyFormula") {
        if (state.holdAction.temp > 0) state.holdAction.temp--;
        if (state.holdAction.delay > 0) {
          ...
          const isApplied = progresscalculation.applyFormulaToState(
            state,
            formula,
            false,
            false,
        ...
        if (state.holdAction?.temp === 0) state.holdAction = null;
      }
      ...
      if (
        ...
        state.millisSinceAutoApply > 1000 / state.autoApplyRate
      ) {
        for (let i = 0; i < 5; i++) {
          if (state.autoApply[i] && state.myFormulas.length > i) {
            progresscalculation.applyFormulaToState(
              state,
              formulaList[state.myFormulas[i]],
              false,
              true,
              true
            );
        ...
        state.millisSinceAutoApply = Math.min(
          100,
          state.millisSinceAutoApply - 1000 / state.autoApplyRate
        ); //Buffer up to 100 ms
      }

      if (recoverHold) state.xValue[recoverTier] = recoverValue;

      if (state.activeChallenges.FORMULAGOD) {
        for (let i = 0; i < 4; i++)
          state.formulaGodScores[i] = Math.max(
            state.formulaGodScores[i],
            state.xValue[i]
          );
      }
      ...
      if (
        state.activeChallenges.COUNTDOWN &&
        state.millisSinceCountdown >= 30000
      ) {
      ...
      if (milestoneList[state.mileStoneCount]?.check(state)) {
        notify.success(
          "New Milestone",
          milestoneList[state.mileStoneCount].name
        );
      ...
      if (destinyMileStoneList[state.destinyMileStoneCount]?.check(state)) {
        notify.success(
          "New Milestone",
          destinyMileStoneList[state.destinyMileStoneCount].name
        );
      ...
      stoneList.forEach((stoneName) => {
        if (
          !state.startingStoneTurned[stoneName] &&
          startingStones[stoneName]?.check(state)
        ) {
          notify.success(
            "Starting Stone Turned",
            startingStones[stoneName].title
          );
      ...
      if (state.justLaunched) {
        state.myFormulas = state.myFormulas.filter(
          (formulaName) => formulaList[formulaName]
        );
        state.holdAction = null;
      }
      ...
      for (
        ;
        state.autoUnlockIndex < shopFormulas.length;
        ...
        if (
          isLockedByChallenge(state, formula) ||
          state.formulaUnlocked[formula.formulaName] ||
          formula.effectLevel > state.highestXTier ||
          formula.targetLevel > state.highestXTier
        )
          continue;
        ...
        if (
          state.xValue[0] <
            formula.unlockCost * getUnlockMultiplier(formula, state) &&
          !formula.isFree
        )
          break;

        //No Auto-Unlock
        if (!state.alphaUpgrades.AUNL) break;
      ...
      if (state.alphaUpgrades.PALP) {
        ...
        if (
          state.passiveAlphaTime >=
          Math.min(state.passiveAlphaInterval, 1000 * 3600 * 24)
        ) {
          state.alpha += Math.floor(
      ...
      const alphaThreshold =
        alphaThresholds[state.settings.alphaThreshold] || alphaTarget;

      if (
        !state.inNegativeSpace &&
        state.settings.autoResetterA !== "OFF" &&
        state.alphaUpgrades.ARES &&
        state.highestXTier === 3 &&
        (state.xValue[0] >= alphaThreshold ||
          (state.insideChallenge && state.xValue[0] >= alphaTarget))
      ) {
        ...
        state.alphaUpgrades.SRES &&
        state.xValue[0] >= differentialTargets[state.highestXTier]
      ) {
        upgradeXTier(state);
        performShopReset(state);
        rememberLoadout(state);
      ...
      if (state.progressionLayer > 0 || state.destinyStars > 1) {
        ...
        for (let i = 0; i < state.xValue.length; i++) {
          perSecond[i] = deltaMilliSeconds
            ? (state.xValue[i] - xValuesBefore[i]) / (deltaMilliSeconds / 1000)
            : 0;
        ...
        for (let i = 0; i < state.xValue.length; i++) {
          ...
          const newAvg =
            (history[2] +
              ...
              history[7]) /
            6;
          state.avgXPerSecond[i] =
            newAvg > state.avgXPerSecond[i] ||
            newAvg < 0.9 * state.avgXPerSecond[i]
              ? newAvg
              : state.avgXPerSecond[i];
      ...
      if (
        deltaMilliSeconds > 120000 &&
        (state.settings.offlineProgressPopup === "ON" ||
          (state.settings.offlineProgressPopup === "LAUNCH" &&
            state.justLaunched))
      ) {
        const timeText = (
          <>
            You were away for{" "}
            {secondsToHms(Math.floor(deltaMilliSeconds / 1000))}
          </>
        );
        const xText = state.insideChallenge ? (
          <></>
        ) : (
          getOfflinePopupLine(
            <>x</>,
        ...
        const aText = getOfflinePopupLine(
          <>&alpha;</>,
          aBefore,
          state.alpha,
          state.settings.numberFormat
        );
        const sText = getOfflinePopupLine(
          <>&lambda;</>,
          sBefore,
          state.starLight,
          state.settings.numberFormat
      ...
      if (
        !state.currentEnding &&
        (isNaN(state.xValue[0]) ||
          isNaN(state.xValue[1]) ||
          isNaN(state.xValue[2]) ||
          isNaN(state.xValue[3]))
      ) {
        performXReset(state);
    ...
    case "swapFormulas":
      ...
      if (state.myFormulas[startIndex] && state.myFormulas[targetIndex]) {
        [state.myFormulas[startIndex], state.myFormulas[targetIndex]] = [
          state.myFormulas[targetIndex],
          state.myFormulas[startIndex],
        ];
      }
      break;
    case "applyFormula":
      if (
        state.activeChallenges.FULLYIDLE ||
        !state.formulaUnlocked[action.formula.formulaName]
      )
        break;
      state.isFullyIdle = false;
    ...
    case "unlockFormula":
      state.isFullyIdle = false;
      const unlockCost = action.formula.isFree
        ? 0
        : action.formula.unlockCost * action.formula.unlockMultiplier;
      if (
        unlockCost > state.xValue[0] ||
        state.formulaUnlocked[action.formula.formulaName]
      )
        break;
      if (!state.alphaUpgrades.AUNL) state.xValue[0] -= unlockCost;
    ...
    case "discardFormula":
      ...
      state.myFormulas = state.myFormulas.filter(
        (formulaName) => formulaName !== action.formula.formulaName
      );
      break;
    case "resetXValues":
      if (
        ...
        state.highestXTier === 0 &&
        state.formulaUnlockCount < 4
      )
        break;
      if (
        state.activeChallenges.FULLYIDLE ||
        state.activeChallenges.ONESHOT ||
        !state.anyFormulaUsed
      )
        break;
      state.isFullyIdle = false;
    ...
    case "alphaReset":
      ...
      if (!action.isAbort) giveAlphaRewards(state);
    ...
    case "memorize":
      state.isFullyIdle = false;
      state.equipLayouts[state.selectedLayout][state.highestXTier] =
        structuredClone(state.myFormulas);
    ...
    case "clearLoadout":
      state.isFullyIdle = false;
      state.myFormulas = state.myFormulas.filter(
        (formulaName) => state.formulaUsed[formulaName]
      );
      state.formulaBought = state.myFormulas.reduce(
        (a, v) => ({ ...a, [v]: true }),
        {}
    ...
    case "toggleAutoApply":
      ...
      if (action.all) {
        ...
        if (state.autoApply.some((b, i) => b && i < getInventorySize(state)))
          state.autoApply = [false, false, false, false, false];
        else state.autoApply = [true, true, true, true, true];
      } else {
    ...
    case "changeFavorite":
      ...
      else delete state.shopFavorites[action.tier][action.formulaName]; //NORMAL
      break;
    case "enterChallenge":
      performAlphaReset(state);
      ...
      if (action.challenge.subChallenges) {
        state.activeChallenges = action.challenge.subChallenges.reduce(
          (a, v) => ({ ...a, [v]: true }),
          {}
        );
      } else {
    ...
    case "startResearch":
      ...
      state.researchLevel[action.research.id] = Math.min(
        2500,
        (state.researchLevel[action.research.id] || 0) + action.bulkAmount
      );
    ...
    case "completeEnding":
      ...
      if (
        isNew &&
        action.endingName !== "world" &&
        endingList[action.endingName]?.titlestring
      )
        notify.success(
          "Ending Completed",
          endingList[action.endingName].titlestring
      ...
      if (
        (action.endingName === "world" ||
          (action.endingName === "true" && state.destinyStars > 1)) &&
        state.progressionLayer < 2
      ) {
        state.progressionLayer = 2;
        state.destinyEndTimeStamp = Date.now();
        if (state.destinyStartTimeStamp > 0)
          state.destinyRecordMillis =
            state.destinyEndTimeStamp - state.destinyStartTimeStamp;
        state.mailsForCheck.push("Destiny");
        notify.success("DESTINY", "You finished the game!");
        performAlphaReset(state);
        performShopReset(state);
      } else if (
        action.endingName === "good" ||
        action.endingName === "evil" ||
        action.endingName === "true" ||
        action.endingName === "skipped" ||
        action.endingName === "world"
      ) {
        performAlphaReset(state);
        performShopReset(state);
    ...
    case "performDestinyReset":
      ...
      state = {
        ...
        shopFavorites: state.shopFavorites,
        mileStoneCount: state.mileStoneCount,
        destinyMileStoneCount: state.destinyMileStoneCount,
        allTimeEndings: state.allTimeEndings,
        destinyStars: state.destinyStars,
        starLight: state.starLight,
        lightAdder: state.lightAdder,
        lightDoubler: state.lightDoubler,
        lightRaiser: state.lightRaiser,
        starConstellations: state.starConstellations,
        constellationCount: state.constellationCount,
        fileStartTimeStamp: state.fileStartTimeStamp,
        destinyStartTimeStamp: Date.now(),
      };
      state.mailsForCheck.push("Destiny");
      break;
    case "buyLightUpgrade":
      ...
      if (action.cost < Infinity) state.starLight -= action.cost;
