...
export default function AlphaResearchBar({ state, research, updateState }) {
  ...
  const progressBarWidth = isDone
    ? "100%"
    : Math.min(100 * percentage, 99).toFixed(2) + "%";
  ...
  if (researchLevel >= 2500) {
    return (
      <>
        ...
        {research.checkBoost2(state) && (
          <div>Special: {research.getBoostText2(state)}</div>
        )}
      </>
    );
  } else {
            ...
            >
              <b>
                {isDone ? (
                  ...
                  secondsToHms(Math.ceil(remainingTime))
                )}
              </b>
            </div>
          </div>
