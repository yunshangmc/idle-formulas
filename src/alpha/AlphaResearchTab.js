...
export const researchDictonary = {
  x: {
    ...
    getBonusText: (level, state) =>
      "Start with x=" +
      formatNumber(
        Math.floor(100 * Math.pow(1.01, level || 0) - 100),
    ...
    getBonusText2: (state) =>
      "Start with x=" +
      formatNumber(10e12, state.settings.numberFormat, 0, true) +
      " after resets",
    getBoostText: (state) => (
      <>
        x={formatNumber(state.xHighScores[0], state.settings.numberFormat)} on
        x'-Reset
      </>
    ),
    getBoostText2: (state) => (
      <>
        x={formatNumber(state.formulaGodScores[0], state.settings.numberFormat)}{" "}
        during Formula God
      </>
    ),
  ...
  "x'": {
    ...
    getBonusText2: (state) =>
      "x' produces " +
      formatNumber(100e9, state.settings.numberFormat, 2, true) +
      " times as fast",
    getBoostText: (state) => (
      <>
        x={formatNumber(state.xHighScores[1], state.settings.numberFormat)} on
        x''-Reset
      </>
    ),
    getBoostText2: (state) => (
  ...
  "x''": {
    ...
    getBoostText: (state) => (
      <>
        x={formatNumber(state.xHighScores[2], state.settings.numberFormat)} on
        x'''-Reset
      </>
    ),
    getBoostText2: (state) => (
  ...
  "x'''": {
    ...
    getBoostText: (state) => (
      <>
        x={formatNumber(state.xHighScores[3], state.settings.numberFormat)} on
        &alpha;-Reset
      </>
    ),
    getBoostText2: (state) => (
...
export default function AlphaResearchTab({
  state,
  updateState,
  setTotalClicks,
}) {
    ...
    <div style={{ marginLeft: "20px" }}>
      {
        <>
          <h2>Research</h2>
          Research speed is boosted by your highscores but higher levels take
          longer.
          {getMaxxedResearchBonus(state).count > 0 && (
            <>
              <br />
              Every maxxed Research Bar doubles your Formula Efficiency (x
              {getMaxxedResearchBonus(state).bonus}).
            </>
          )}
          <br />
          <br />
