...
export default function EndingFinalScreen({
  state,
  action,
  popup,
  updateState,
  ...
  const buttonsVisible =
    action.instaDestiny ||
    getGlobalMultiplier(state) * (Date.now() - startTime) > 10000;
  const changeDestiny = () => {
    ...
    >
      {
        <>
          ...
          <br />
          <p>
            <b>{action.headerText}</b>
          </p>
          <button
            onClick={changeDestiny}
            className="fbutton"
            style={{
              visibility: !buttonsVisible ? "hidden" : undefined,
              width: "250px",
            }}
          >
            <b>{action.buttonText || "CHANGE YOUR DESTINY"}</b>
          </button>
          <br />
          <br />
          <button
            onClick={alternativeDestiny}
            style={{
              visibility:
                !buttonsVisible || !action.buttonText2 ? "hidden" : undefined,
              width: "200px",
            }}
          >
            {action.buttonText2}
