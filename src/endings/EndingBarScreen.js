...
export default function EndingBarScreen({ state, popup, updateState }) {
  ...
  if (currentAction.hesitance) {
    if (percentage < 0.8) hesitancePercentage = percentage;
    else if (percentage < 0.975) hesitancePercentage = 0.8;
    else hesitancePercentage = 0.8 + 8 * (percentage - 0.975);
  }
  const progressBarWidth = isDone
    ? "100%"
    : Math.min(101 * hesitancePercentage, 100).toFixed() + "%";

  const currencyPerSecond = ending.generators
    ? ending.generators.reduce(
        (a, v) => a + (generatorAmounts[v.title] || 0) * v.production,
        0
      )
    : 0;
  const cpsMultiplier = Math.max(
    0,
    Math.min(1, 1.49 - (1.5 * currencyAmount) / ending.currencyGoal)
  );
  ...
  if (deltaUpdate > 100 && !ending.valueBased) {
    ...
    newCurrencyAmount +=
      cpsMultiplier * ((deltaUpdate * currencyPerSecond) / 1000);
  }

  if (startTime && isDone) {
    if (ending.valueBased) {
      newCurrencyAmount = currentAction.getValue
        ? currentAction.getValue(repetitionAmount + 1)
        : newCurrencyAmount;
    } else {
    ...
    if (currentAction.split) {
      newCurrencyAmount = Math.floor(
        currencyAmount +
          (ending.currencyGoal - currencyAmount) / currentAction.split
      );
    }

    setStartTime(0);
    if (currentAction.generator) {
      generatorAmounts[currentAction.generator] =
        (generatorAmounts[currentAction.generator] || 0) + 1;
      setGeneratorAmounts(generatorAmounts);
    }

    if (
      repetitionAmount >= (currentAction.repeat || 0) &&
      (!currentAction.requirement ||
        newCurrencyAmount >= currentAction.requirement)
    ) {
  ...
  const clickProgressBar = () => {
    if (currentAction.finishOnClick) {
      updateState({
        name: "completeEnding",
        endingName: currentAction.endingName,
      });
      return;
    }
    ...
    >
      {
        <>
          ...
          <br />
          {(currencyAmount > 0 || currencyAmount.length > 0) && (
            <p>
              {ending.currencyName}
              {ending.ascending === -1
                ? Math.ceil(ending.currencyGoal - currencyAmount)
                : currencyAmount}
            </p>
          )}
          <br />
          ...
          >
            <div
              style={{
                backgroundColor: currentAction.barColor || "#aaaaaa",
                border: "0px",
                height: "25px",
          ...
          {ending.generators.map((generator) => (
            <div
              style={{
                visibility:
                  generatorAmounts[generator.title] !== undefined
                    ? undefined
                    : "hidden",
              }}
              key={generator.title}
            >
              {generator.title}:{" "}
              {Math.ceil(
                generatorAmounts[generator.title] * generatorMultiplier
              )}
              <br />
            </div>
          ))}
          <div style={{ visibility: currencyPerSecond ? undefined : "hidden" }}>
            Total {ending.productionName}:{" "}
            {(currencyPerSecond * cpsMultiplier).toFixed(2)}
            <br />
          </div>
        </>
      }
