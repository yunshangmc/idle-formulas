...
export default function Mail({ state, mail, popup, updateState, mailid }) {
  const [isUnread] = useState(
    state.mailsUnread[mailid] || !state.mailsReceived[mailid]
  );

  if (!state.mailsReceived[mailid]) return undefined;
    ...
    if (mail.exercises[eindex].correct === aindex) {
      ...
      for (let i = 0; i < mail.exercises.length; i++) {
        ...
        for (let j = 0; j < answersheet.length; j++) {
          ...
          if (answersheet[j]) {
            totalCount++;
            if (exercise.correct === j) correctCount++;
          }
        }
      }
      if (correctCount === mail.exercises.length) {
        ...
        else updateState({ name: "completeMail", mailid, reply: 3 }); //Terrible
      }
    } else {
      updateState({
        name: "progressMail",
  ...
  if (
    ...
    (!mail.afterComplete && !mail.responses)
  )
    displayColor = "#666666"; //grey
  if (isUnread) displayColor = "#FFAA66"; //orange

  if (mail.alphaCost > 0 && !state.mailsUnlocked[mailid])
    return (
      <details style={{ paddingTop: "10px", color: displayColor }}>
        ...
        <p style={{ paddingLeft: "30px" }}>
          <button disabled={state.alpha < mail.alphaCost} onClick={unlockMail}>
            UNLOCK MAIL CONTENT
          </button>
          &nbsp;&nbsp;Cost: &alpha; ={" "}
          {formatNumber(mail.alphaCost, state.settings.numberFormat, 0)}
        </p>
      </details>
    );
  ...
  return (
    <details style={{ paddingTop: "10px", color: displayColor }}>
      ...
      <p style={{ paddingLeft: "30px" }}>{mail.content}</p>

      {mail.rating &&
        (state.mailsCompleted["Survey"] !== undefined ? (
          ...
          <p style={{ paddingLeft: "50px", fontWeight: 900 }}>
            ...
            <br />
            <br />
            <button onClick={submitRating} style={{ marginLeft: "100px" }}>
              Submit
            </button>
          </p>
        ))}
      ...
      {mail.surveyresult && (
        <p style={{ paddingLeft: "50px" }}>
          ...
          <br />
          Your rating: {state.mailsProgress["Survey"].toFixed(0)} / 5 stars.
          <br />
          Average rating: {getAverageRating().toFixed(2)} / 5 stars.
          <br />
          <br />
          {getAverageRating() >= 10 && (
            <>
              This gotta be the best game of the world!
              <br />
            </>
          )}
          {getAverageRating() >= 4 && (
          ...
          {getAverageRating() > 1 && getAverageRating() < 4 && (
            <>
              I was hoping it would score a little better!
              <br />
            </>
          )}
          {getAverageRating() <= 1 && (
            <>
              Oh no, my game is horrible!
              <br />
            </>
          )}
      ...
      {mail.exercises &&
        mail.exercises.map((exercise, eindex) => (
          <p
            style={{
              paddingLeft: "50px",
              fontWeight: exercise.important ? 900 : undefined,
            }}
            key={eindex}
          >
            {exercise.question}&nbsp;=&nbsp;
            {state.mailsProgress[mailid][eindex][exercise.correct]
              ? exercise.answers[exercise.correct]
              : "?"}
            &nbsp;
            {!state.mailsProgress[mailid][eindex][exercise.correct] && (
              <span style={{ display: "inline-block" }}>
                {exercise.answers.map((answer, aindex) => (
                  <button
                    key={aindex}
                    disabled={state.mailsProgress[mailid][eindex][aindex]}
                    onClick={() => selectAnswer(eindex, aindex)}
      ...
      {mail.transfer &&
        (state.mailsCompleted["Transfer"] === undefined ? (
          <p style={{ paddingLeft: "50px", fontWeight: 900 }}>
            <span
              className="fbutton"
              style={{ width: "auto", backgroundColor: "#ffffff" }}
              onClick={() => changeRating(+1)}
            >
              Transfer 1$
            </span>
            {state.mailsProgress["Transfer"] > 0 && (
              <>
                <br />
                <br />${state.mailsProgress["Transfer"]} Transferred
              </>
            )}
          </p>
        ) : (
          <p style={{ paddingLeft: "50px", fontWeight: 900 }}>
            {state.mailsProgress["Transfer"] > 0 && (
              <>${state.mailsProgress["Transfer"]} Transferred</>
            )}
          </p>
        ))}
      ...
      {state.mailsCompleted[mailid] === undefined ? (
        !mail.getProgress && (
          <p style={{ paddingLeft: "30px" }}>
            {mail.responses
              ?.slice(0, mail.responses.length - (mail.hiddenResponses || 0))
              .map((response, index) => (
                <button
                  key={index}
                  onClick={() => completeMail(index)}
