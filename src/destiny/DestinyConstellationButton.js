...
export default function DestinyConstellationButton({
  state,
  popup,
  constellation,
  updateState,
  ...
  if (
    !state.starConstellations[constellation.id] &&
    state.starLight < constellationPrices[state.constellationCount]
  )
    return constellation.fixed;

  const clickConstellation = () => {
    if (!state.starConstellations[constellation.id]) {
      popup.confirm(
        <>
          Use all Starlight and sacrifice all Starlight Upgrades to complete the{" "}
          {constellation.title} Constellation?
        </>,
        () => {
          console.log("Clicked on" + constellation.title);
          updateState({
            name: "completeConstellation",
